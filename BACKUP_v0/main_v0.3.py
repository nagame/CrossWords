...
class Dictionary:
    ...
    def __init__(self, source_file=None):
        ...
        self._load_dictionary(source_file)

    def _load_dictionary(self, source_file):
        """
        ...
        print('Dictionary referenced, object structure loading time:   ', time.perf_counter() - timing)

        # report memory usage
        mem_use_kb = psutil.Process(os.getpid()).memory_info()[0]/1024  # memory usage by the process [kb]
        print('Process memory, after loading dictionary referenced, object structures:', mem_use_kb, 'kb')
    ...
    def origin_words(self, sequence, pos=None, length=None):
        """
        ...
        if sequence in self.dictionary['words']:
            words[sequence] = self.dictionary['words'][sequence]
