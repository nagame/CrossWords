...
class Sequence:
    ...
    def inWord(self, word):
        ...
        if self.position is None:
            return True
        # sequenc specifies position, check it
        w_strip = word
        pos_ok = False
        while self.position >= w_strip.find(str(self.sequence)) > -1:
            if self.position == w_strip.find(str(self.sequence)):
                pos_ok = True
                break
            else:
                w_strip = w_strip[w_strip.find(str(self.sequence)) + 1:]
                # translate position because current word got cut
                self.position -= w_strip.find(str(self.sequence)) + 1
                if w_strip.find(str(self.sequence)) == self.position:
                    pos_ok = True
                    break
        return pos_ok
...
class Dictionary:
    ...
    def match_words(self, *args, length=None):
        ...
        if length is not None:
            ...
            shortest_seq_words = words_filtered
...
'aberration' in D.match_words(Sequence('ab', 0))
