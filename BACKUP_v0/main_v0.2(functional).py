    ...
    for seq_len in range(s1, s2):
        for idx in range(len(letters)):
            ...
            try:
                ...
                try:
                    dictionary['letters'+str(seq_len)][seq_key] += 1
...
try:
    dictionary['letters1'] = {k: v for k, v in sorted(dictionary['letters1'].items(), key=lambda item: item[1], reverse=True)}
...
dictionary['words'] = {k: v for k, v in
                          sorted(dictionary['words'].items(), key=lambda item: item[1].seq_num, reverse=True)}
