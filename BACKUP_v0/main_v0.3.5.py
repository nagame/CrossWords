...
source_file = open("small.txt", encoding="utf-8")
D = Dictionary(source_file, depth=(1, 15))
d = D.dictionary
S = Sequence
...
for letter in solution:
    solution_position = randint(1, 10)  # which letter in answer belongs to solution
    possible_words = D.match_words(S(letter, solution_position))
    # chosen_word = possible_words[randint(0, len(possible_words)-1)]
    chosen_word = random.choice(list(possible_words.keys()))
    print(f'Word: {chosen_word}, position: {solution_position}')
    wordy = ''.join([
                        chosen_word[n].upper() if n == solution_position else chosen_word[n]
