...
class CrossWord:
    def __init__(self, *args, **kwargs):
        ...
        self.match = self._match  # match  a word from dictionary to the restrictions
    ...
    def _match(self):
        ...
        """
        1. Translate cells content into sequences
        2. use all sequences to find a word from dictionary
        """

        # translate self.cells into sequences and add to self.sequences
        found_str = ''
        for cell_num in range(len(self.cells)):
            ...
            # this cell has no letter - create sequence object if string found
                ...
                if len(found_str) > 0:
                    self.sequences.append(Sequence(found_str, cell_num))  # add sequence
                    found_str = ''
...
# sample cells: 5 letter word, starting at (0,0), going down
coords = [(x, 0) for x in range(6)]
cells = [grid[c[0]][c[1]] for c in coords]
cells[0].content['letter'] = 'm'
cells[1].content['letter'] = 'a'
cells[3].content['letter'] = 's'

cw = CrossWord(cells=cells)
