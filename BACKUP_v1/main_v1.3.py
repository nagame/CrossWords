...
class CrossWord:
    """
    Single word in a crosswords diagram: contains Cell(s), Sequence(s)
    """
    def __init__(self, *args, **kwargs):
        # one or more Sequence objects to match into the word
        self.sequences = kwargs['sequences'] if 'sequences' in kwargs else []
        ...
        self.dictionary = kwargs['dictionary'] if 'cells' in kwargs else None

        # matching words (constrained by sequences and cells)
        self.matching_words = None

        self.match = self._match  # match  words from dictionary to the restrictions
        self.random_match = self._random_match  # match potential words and fill the cells with it
    ...
    def _cells_to_sequences(self):
        ...
        for cell_num in range(len(self.cells)):
            ...
            # this cell has no letter - create sequence object if string found
                ...
                if len(found_str) > 0:
                    self.sequences.append(Sequence(found_str, start_pos))  # add sequence
                    found_str = ''
    ...
    def _match(self):
        ...
        matched_words = self.dictionary.match_words(*self.sequences, length=len(self.cells))
    ...
    def print_grid():
        ...
        for y in range(grid_size[1]):
            ...
            for x in range(grid_size[0]):
                gridline += (str(grid[x][y])+' ') if grid[x][y] is not None else ' '
            g.append(gridline)
        g.reverse()
        for line in g:
            print(line)
